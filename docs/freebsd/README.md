...
Some additional steps are required on the host in order to use bhyve.  First, ensure that the host system is at r346550 or later.  Second, since bhyve currently does not support disk image snapshots, ZFS must be used to provide equivalent functionality.  Create a ZFS data set for the syzkaller workdir and copy the VM image there:
```console
# zfs create data/syzkaller
# cp FreeBSD.raw /data/syzkaller
# zfs snapshot data/syzkaller@1
```
Finally, configure networking and DHCP for the VM instances:

```console
# ifconfig bridge create
bridge0
# ifconfig bridge0 inet 169.254.0.1
# echo 'dhcp-range=169.254.0.2,169.254.0.254' > /usr/local/etc/dnsmasq.conf
