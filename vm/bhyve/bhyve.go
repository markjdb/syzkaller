...
type Config struct {
	...
	Snapshot string `json:"snapshot"` // ZFS snapshot of workdir
}
...
func (pool *Pool) Create(workdir string, index int) (vmimpl.Instance, error) {
	inst := &instance{
		...
		workdir: pool.env.Workdir,
		vmName:  fmt.Sprintf("syzkaller-%v-%v", pool.env.Name, index),
	}

	ds := strings.Split(inst.cfg.Snapshot, "@")
	if len(ds) != 2 {
		return nil, fmt.Errorf("invalid snapshot name %v", inst.cfg.Snapshot)
	}

	cloneName := fmt.Sprintf("%v/bhyve%v", ds[0], inst.vmName)
	osutil.RunCmd(time.Minute, "", "zfs", "destroy", cloneName)
	cloneArgs := []string{
		"clone",
		inst.cfg.Snapshot,
		cloneName,
	}
	if _, err := osutil.RunCmd(time.Minute, "", "zfs", cloneArgs...); err != nil {
		inst.Close()
		return nil, err
	}
	inst.clone = cloneName
...
func (inst *instance) Boot() error {
	image := fmt.Sprintf("%v/bhyve%v/vm.raw", inst.workdir, inst.vmName)
	loaderArgs := []string{
		"-c", "stdio",
		"-m", inst.cfg.Mem,
		"-d", image,
	...
	bhyveArgs := []string{
		...
		"-s", fmt.Sprintf("3:0,virtio-blk,%v", image),
		"-l", "com1,stdio",
		inst.vmName,
	}
...
func (inst *instance) Close() {
	...
	if inst.clone != "" {
		osutil.RunCmd(time.Minute, "", "zfs", "destroy", inst.clone)
		inst.clone = ""
	}
	if inst.tapdev != "" {
